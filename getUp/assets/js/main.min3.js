...
var _lang = {
    zh: {
        title: "",
        help_txt: "",
        score: "<img src='assets/img/gk.png'>",
        btn_pause: "暂停",
        btn_start: "<img style='width:55%' src='assets/img/an_10.png'>",
        btn_reTry: "<img src='assets/img/an_12.png'>",
        btn_more_game: "更多游戏",
        game_pause: "游戏暂停",
        btn_resume: "继续游戏",
        loading: "加载中...",
        lv_txt: ["色狼", "色鬼", "色魔", "超级色魔", "变态色魔", "孤独求色",],
        share_txt1: "【微信找你妹】我在",
        share_txt2: "只袜子中找到",
        share_txt3: "个妹子，我是【",
        share_txt4: "】，不服来战！",
...
function() {
    ...
    c = {
        ...
        gameOver: function(zt) {
            ...
            b.d_content.hide(),
            b.d_gameover.show().find("h3").text((this.lastGameTxt-1));
			if($("#room header .lv em").html()>zg){
				zg=$("#room header .lv em").html()-1;
			}
			if(tx==1){
	            b.d_gameover.show().find("h4").text(zg).addClass("zgf");
				$(".btn-boyaa").hide();
				$(".btn-tx").show();
			}
			else{			
	            b.d_gameover.show().find("h4").text(this.lastGameTxt-1)
			};
			clearTimeout(clearTime);
			if(zt==1){
				$("#box .spanD").removeClass("spanD");
				$(".timeDown").addClass("boomS");
				$(".boom").addClass("boom2");
				a.find("span").delay(1500).fadeOut(1500,
				function() {
					$(".boom").removeClass("boom2");
					b.dialog.show();
					$("#room header .lv em").html(0);
				})
			}	
            else{
        ...
        render: function() {
            setTimeout(function() {
				...
				if(played==1){
					$(".pause").hide();
					$("#eplay").show();
					$("#eplay .inner h4").html(zg);
					if(tx==1){
						$("#eplay .btn-boyaa").hide();
						$("#eplay .btn-tx").show();
					}
					return false;
				}
...
function() {
    ...
    e = {
        ...
        render: function(e, f) {
            ...
            var size= a.find(b).height()*0.4;
            a.find(b).data("type", "b").html('<span style="padding:0px;border:0;font-size:'+size+'px;line-height:'+(size/0.4)+'px;width:100%;height:100%" >55</span>'),
            a.find(b).eq(h).data("type", "a").html('<span class="timeDown" style="padding:0px;border:0;font-size:'+size+'px;line-height:'+(size/0.4)+'px;width:100%;height:100%" ></span>'),
			clearTimeout(clearTime),
			time=9-Math.round($("#room header .lv em").html()/2);
			if(time<=5){
				time=5;	
			}
			timeDown();//倒计时
...
$(function(){
	...
	$(".fxPop").click(function(){
		$(".fxPop").fadeOut();						   
	})
	
	$(".gz img").click(function(){
		$(".gzPop").fadeIn();
	})
	$(".gzPop").click(function(){
		$(".gzPop").fadeOut();
	})
...
function timeDown(){
	$(".timeDown").html("0"+time--);
	clearTime=setTimeout("timeDown()",1000);
	if(time<=2){
		$("#box span").addClass("spanD");	
	}
	if(time<0){
		clearTimeout(clearTime);
		window.Game.gameOver(1);
		return false;
	}
}